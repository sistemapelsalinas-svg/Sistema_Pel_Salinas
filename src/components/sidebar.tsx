...
export function Sidebar({ isMobileOpen = false, onClose }: SidebarProps) {
  ...
  // Conteúdo unificado da Sidebar
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white dark:bg-[#151A23] text-gray-700 dark:text-gray-200 select-none">
                ...
                if (item.onClick) {
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-xs font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100/70 dark:hover:bg-[#1D2432] hover:text-gray-900 dark:hover:text-white transition-all text-left group"
                    >
                      <div className="flex items-center gap-2.5">
  ...
  return (
    <>
      {/* 1. Desktop Sidebar (Fixo e visível apenas em telas grandes lg:flex) */}
      <aside className="hidden lg:flex w-64 flex-col flex-shrink-0 border-r border-gray-200/90 dark:border-[#222938] sticky top-0 h-screen z-30">
        {sidebarContent}
      </aside>

      {/* 2. Mobile Drawer Off-Canvas (Abre e fecha suavemente em telas pequenas) */}
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-72 max-w-[85vw] bg-white dark:bg-[#151A23] shadow-2xl z-50 animate-in slide-in-from-left duration-300">
            {sidebarContent}
          </div>
        </div>
      )}
